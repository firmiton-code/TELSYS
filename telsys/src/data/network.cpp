...
void NetworkClass::setupServer(){
  ...
  server.on("/get", HTTP_GET, [] (AsyncWebServerRequest *request) {
      ...
      if (request->hasParam("ssid")) {
        ...
        data.save("ssid", net_instances->ssid);
      ...
      if (request->hasParam("password")) {
        ...
        data.save("pass", net_instances->pass);
        debugVal(NET_TAG, "Password : ", net_instances->pass);
      }

      if (request->hasParam("uid")) {
        inputMessage = request->getParam("uid")->value();
        inputParam = "uid";
        net_instances->uid = inputMessage;
        data.save("uid", net_instances->uid);
        debugVal(NET_TAG, "UID : ", net_instances->uid);
      }
      
      if(net_instances->ssid != "" && net_instances->pass != "" && net_instances->uid != ""){
...
void NetworkClass::reset(){
  ...
  String buff_uid = data.read("uid");
  data.format();
  data.save("uid", buff_uid);
}
...
void NetworkClass::begin(){
  ...
  if(data.read("ssid")==""){
    ...
    net_instances->uid = data.read("uid");
  }
...
String NetworkClass::get_uid(){
    return net_instances->uid;
}
